...
class TestHttpStatusCode(unittest.TestCase):
    def test_enum_values(self):
        # Проверяем, что коды и сообщения соответствуют ожиданиям
    ...
    def test_get_message_by_code_existing(self):
        # Проверяем, что метод возвращает корректные сообщения для существующих кодов
    ...
    def test_get_message_by_code_non_existing(self):
        # Проверяем, что метод возвращает сообщение для несуществующего кода
        self.assertEqual(HttpStatusCode.get_message_by_code(999), "Unknown Status Code")

    def test_code_and_message_properties(self):
        # Проверяем, что свойства code и message возвращают ожидаемые значения
